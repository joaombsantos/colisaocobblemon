...
public class SurfHandler {
        ...
        for (ServerPlayer player : world.players()) { // getPlayers -> players
            ...
            if (isInWater) {
                if (!hasSurfItem(player)) {
                    ...
                    player.sendSystemMessage( // sendMessage -> sendSystemMessage
                            Component.translatable("message.colisao-cobblemon.need_surf").withStyle(ChatFormatting.RED)
                    );
                }
            }
            // --- JOGADOR NA TERRA ---
            else if (player.onGround()) { // isOnGround -> onGround
    ...
    private static boolean isStrictlySafe(ServerLevel world, BlockPos pos) {
        ...
        if (isWater(world, pos.below())) return false; // down -> below

        // 3. Verificação de VIZINHOS (Detector de Barranco)
        for (Direction dir : Direction.Plane.HORIZONTAL) { // Direction.Type.HORIZONTAL -> Direction.Plane.HORIZONTAL
            BlockPos neighbor = pos.relative(dir); // offset -> relative
    ...
    private static boolean isWater(ServerLevel world, BlockPos pos) {
        return world.getFluidState(pos).is(FluidTags.WATER); // isIn -> is
    }
    ...
    private static Vec3 findNearestLand(ServerLevel world, BlockPos center) {
        ...
        for (int x = -3; x <= 3; x++) {
            for (int y = -2; y <= 3; y++) {
                for (int z = -3; z <= 3; z++) {
                    BlockPos checkPos = center.offset(x, y, z); // add -> offset

                    if (world.getBlockState(checkPos).isSolidRender(world, checkPos) && // isSolidBlock -> isSolidRender (mais seguro)
                            !isWater(world, checkPos.above()) && // up -> above
                            !isWater(world, checkPos) &&
                            world.getBlockState(checkPos.above()).isAir()) {

                        double dist = center.distSqr(checkPos); // getSquaredDistance -> distSqr
                        if (dist < bestDist) {
                            bestDist = dist;
                            bestPos = checkPos;
                        }
    ...
    private static boolean hasSurfItem(ServerPlayer player) {
        ...
        return inv.contains(HmManager.SURF.getDefaultInstance()); // getDefaultStack -> getDefaultInstance
    }
}